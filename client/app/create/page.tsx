...
export default function CreateStoryPage() {
	...
	const handleCreateStory = async () => {
		if (!title || !description || !genre || totalPercentage !== 100) {
			alert("Please fill all fields and make sure revenue splits total 100%")
			return
		}
		...
		const storyData = {
			...
			contributors,
			voting_duration: Number(votingDuration),
			quorum_threshold: Number(quorumThreshold),
			is_public: isPublic,
		}

		try {
			const { data, error } = await supabase.from("stories").insert([storyData])
			if (error) {
									...
									<div className="space-y-2">
										...
										<Select value={genre} onValueChange={setGenre}>
											...
											<SelectContent className="bg-white dark:bg-gray-900 border border-border rounded-md shadow-lg">
												{genres.map((g) => (
													<SelectItem key={g} value={g} className="hover:bg-primary/10 data-[highlighted]:bg-primary/20">
														{g}
													</SelectItem>
												))}
											</SelectContent>
